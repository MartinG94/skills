...
| [`organization-analysis`](organizationAnalysis/SKILL.md) | se define la organización bajo estudio (misión, visión, SMART, cliente) y analiza la Cadena de Valor Virtual (Rayport & Sviokla) en Etapa 1 con las 3 fases de madurez digital | narrativa operativa, datos brutos y modelo de negocio | matriz canónica en 'cadena_valor_virtual.md' con Encuadre Matriz 1, cruce físico-virtual y diagnóstico SDL |
| [`process-map`](processMap/SKILL.md) | se construye, clasifica y valida el Mapa de Procesos Institucional en los 3 niveles canónicos (Estratégicos, Operativos y Soporte) con su diagrama visual Mermaid, y se evalúa y prioriza el proceso crítico a intervenir mediante los 5 factores de cátedra con su justificación técnica | relevamiento institucional, áreas funcionales, objetivos estratégicos y dolores operativos | mapa canónico en 'mapa_procesos.md', bloque Mermaid con proceso crítico resaltado, matriz multicriterio ponderada y justificación técnica |
...
| [`cameStrategizer`](cameStrategizer/SKILL.md) | se construyen y auditan los cruces estratégicos de la Matriz CAME (FO, FA, DO, DA) a partir del diagnóstico FODA | matriz FODA del proceso validada y objetivos estratégicos | matriz formal de cruces estratégicos CAME en 'came.md' |
...
1. **Etapa 1 — Situación Actual y Selección Ponderada del Proceso Crítico:**
   ...
   - `organizationAnalysis`: formaliza la definición institucional (Misión, Visión, Objetivos SMART, Segmento de Clientes, Propuesta de Valor), modela la Cadena de Valor Virtual de 5 etapas (Recopilar, Organizar, Seleccionar, Sintetizar, Distribuir) cruzada con la cadena física (La Matriz del Valor) y clasifica la madurez digital en 3 fases de cátedra (Visibilidad, Capacidad de Reflejo, Matriz del Valor), guardando deterministamente `cadena_valor_virtual.md`.
   - `processMap`: construye y valida el Mapa de Procesos institucional estructurado en 3 niveles canónicos (Estratégicos, Clave/Operativos, Soporte) con diagrama Mermaid embebido (destacando el proceso crítico), requisitos y satisfacción del cliente, tabla descriptiva con objetivos y dueños, y ejecuta la evaluación multicriterio ponderada con los 5 factores de cátedra para dictaminar el proceso crítico a intervenir y la justificación técnica de su selección, guardando deterministamente `mapa_procesos.md`.
   ...
   - `diagramStudio`: exporta o renderiza el Mapa de Procesos y diagramas complementarios en formato editable `.drawio` y/o Mermaid interactivo.
